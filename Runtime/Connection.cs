...
namespace Best.SignalR
{
    ...
    public sealed class Connection : IHeartbeat, IConnection
    {
        ...
        private void OnNegotiationDataReceived(NegotiationData data)
        {
            ...
            TransportConnectionStartedAt = DateTime.Now;
        ...
        public void Reconnect()
        {
            ...
            if (this.State != ConnectionStates.Reconnecting)
                ReconnectStartedAt = DateTime.Now;
        ...
        void IConnection.Error(string reason)
        {
            ...
            {
                this.ReconnectDelayStartedAt = DateTime.Now;
                if (this.State != ConnectionStates.Reconnecting)
                    this.ReconnectStartedAt = DateTime.Now;
        ...
        void IHeartbeat.OnHeartbeatUpdate(DateTime now, TimeSpan dif)
        {
            switch(this.State)
            {
                case ConnectionStates.Connected:
                    if (Transport.SupportsKeepAlive && NegotiationResult.KeepAliveTimeout != null && now - LastMessageReceivedAt >= NegotiationResult.KeepAliveTimeout)
                        Reconnect();

                    if (PingRequest == null && now - LastPingSentAt >= PingInterval)
                        Ping();

                    break;

                case ConnectionStates.Reconnecting:
                    if (now - ReconnectStartedAt >= NegotiationResult.DisconnectTimeout)
                    {
                ...
                default:
                    ...
                    if (TransportConnectionStartedAt != null && now - TransportConnectionStartedAt >= NegotiationResult.TransportConnectTimeout)
                    {
